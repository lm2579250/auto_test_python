...
class Common(object):
    ...
    def get_path(*args):
        ...
        try:
            path = str(os.path.dirname(os.path.dirname(__file__)))  # 获取当前目录的父目录路径(项目根路径)

            for value in args:  # 循环生成路径
    ...
    def get_result_path(self, *args):
        ...
        result_path = Common.get_path("result", self.start_time, *args)  # 自动生成result父目录和时间子目录，以及时间目录下的各级子目录
        return result_path

    def get_api_cases(self):
        """从Excel中读取api cases"""
        try:
            self.api_cases_path = Common.get_path("data", "api_cases.xlsx")  # 拼接接口用例api_cases.xlsx路径,文件后缀只能是".xlsx"
            wb = openpyxl.load_workbook(self.api_cases_path)  # 打开xls文件
            sheets = wb.sheetnames  # 获取workbook中所有的sheet
            ...
            # origin = ""  # api用例请求地址原点(headers中的内容)
            ...
            for i in range(len(sheets)):  # 循环遍历所有sheet
                ...
                origin = sheet.cell(row=1, column=1).value  # 获取第一行第一列的api用例请求地址原点(headers中的内容)
