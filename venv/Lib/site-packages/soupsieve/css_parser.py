...
ATTR = r'''
(?:{ws}*(?P<cmp>[!~^|*$]?=){ws}*(?P<value>{value})(?:{ws}+(?P<cases>[is]))?)?{ws}*\]
...
class CSSParser(object):
    ...
    def parse_attribute_selector(self, sel, m, has_selector, quirks):
        ...
        inverse = False
        op = m.group('cmp')
        case = util.lower(m.group('cases')) if m.group('cases') else None
    ...
    def parse_has_combinator(self, sel, m, has_selector, selectors, rel_type, index):
        ...
        if combinator == COMMA_COMBINATOR:
            ...
            if has_selector:
                ...
                # then we've hit the multiple combinator cases, so we should fail.
                raise SelectorSyntaxError(
