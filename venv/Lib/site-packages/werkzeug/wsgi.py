...
def get_input_stream(environ, safe_fallback=True):
    ...
    # that cases we return the stream unchanged as we know we can safely
...
def get_query_string(environ):
    ...
    # In that cases we want to urllib quote it badly.
...
def extract_path_info(environ_or_baseurl, path_or_url, charset='utf-8',
    ...
    :param path_or_url: an absolute path from the server root, a
                        relative path (in which cases it's the path info)
...
class ProxyMiddleware(object):
    ...
    ``ssl_context``:
        In cases this is an HTTPS target host then an SSL context can be
        provided here (:class:`ssl.SSLContext`).  This can be used for instance
        to disable SSL verification.

    In this cases everything below ``'/static/'`` is proxied to the server on
...
class LimitedStream(io.IOBase):
    ...
    def on_disconnect(self):
        ...
        value of this function is returned from read functions in cases
