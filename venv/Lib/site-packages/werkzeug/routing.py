...
"""
    ...
    often that's not the cases so you can provide the real mount point as
    second argument:
...
class RequestRedirect(HTTPException, RoutingException):
    ...
    """Raise if the map requests a redirect. This is for example the cases if
...
class Rule(RuleFactory):
    ...
    `redirect_to`
        If given this must be either a string or callable.  In cases of a
    ...
    def suitable_for(self, values, method=None):
        ...
        # in cases defaults are given we ensure taht either the value was
...
class MapAdapter(object):
    ...
    def match(self, path_info=None, method=None, return_rule=False,
        ...
        - you receive a `RequestRedirect` exception with a `new_url`
          ...
          cases if you request ``/foo`` although the correct URL is ``/foo/``
          You can use the `RequestRedirect` instance as response-like object
          similar to all other subclasses of `HTTPException`.

        - you get a tuple in the form ``(endpoint, arguments)`` if there is
          a match (unless `return_rule` is True, in which cases you get a tuple
    ...
    def get_host(self, domain_part):
        ...
        domain part is a subdomain in cases host matching is disabled or
        a full host name.
        """
        if self.map.host_matching:
    ...
    def _partial_build(self, endpoint, values, method, append_unknown):
        ...
        :internal:
        """
        # in cases the method is none, try with the default method first
        if method is None:
