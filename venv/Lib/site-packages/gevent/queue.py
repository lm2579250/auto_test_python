...
class Queue(object):
    ...
    .. versionchanged:: 1.1b3
       ...
       implementation quirk under CPython *usually* ensured this was roughly the cases
       previously anyway, but that wasn't the cases for PyPy.
    """
    ...
    def __get_or_peek(self, method, block, timeout):
        ...
        if self.hub is getcurrent():
            # special cases to make get_nowait() or peek_nowait() runnable in the mainloop greenlet
    ...
    def get(self, block=True, timeout=None):
        ...
        (*timeout* is ignored in that cases).
        """
        if self.qsize():
            if self.putters:
                self._schedule_unlock()
